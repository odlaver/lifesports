<section class="table-container catalog-dashboard">
                ...
                <form method="GET" action="<?= BASEURL; ?>/pelanggan/lapangan" id="filterForm">
                    <div class="filter-bar compact" id="filter-lapangan-top">
                        <input class="form-control" name="search" id="search-lapangan" type="text" placeholder="Cari nama lapangan..." value="<?= htmlspecialchars($data['filter']['search']); ?>">
                        <select class="form-control" name="sport" id="filter-cabang">
                        ...
                        <select class="form-control" name="location" id="filter-lokasi">
                        ...
                        <input class="form-control" name="date" id="filter-tanggal" type="date" value="<?= htmlspecialchars($data['filter']['date']); ?>">
                        <select class="form-control" name="sort" id="filter-sort">
                ...
                <div class="catalog-grid">
                    <?php if (empty($data['lapangan'])): ?>
                        <div class="grid-full text-center p-30 text-muted-color">
                            <i class="fas fa-search-minus fa-3x mb-12 d-block text-gold-color"></i>
                        ...
                        ?>
                            <article class="venue-card">
                                <img src="<?= strpos($foto, 'http') === 0 ? $foto : BASEURL . '/assets/uploads/lapangan/' . $foto; ?>" 
                                     alt="<?= htmlspecialchars($lapangan['nama_lapangan']); ?>" class="venue-img">
                                <div class="venue-body">
                                    <div class="venue-topline">
                                        <span class="sport-chip <?= $chip_class; ?> font-size-xs m-0 px-12 py-12"><?= htmlspecialchars($lapangan['nama_kategori']); ?></span>
                                        <strong><i class="fas fa-star text-gold-color"></i> <?= number_format($lapangan['rating'], 1); ?></strong>
                                    </div>
                                    <h2><?= htmlspecialchars($lapangan['nama_lapangan']); ?></h2>
                                    <p><i class="fas fa-map-marker-alt"></i> <?= htmlspecialchars($lapangan['lokasi'] ?? 'Bandar Lampung'); ?></p>
