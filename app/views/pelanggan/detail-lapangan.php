<!DOCTYPE html>
<html lang="id">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Detail Lapangan - Lifesports</title>
    <link rel="stylesheet" href="<?= BASEURL; ?>/public/assets/css/styles.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
</head>

<body>
    <header class="navbar">
        <a href="../public/index.html" class="nav-brand">
            <div class="brand-icon">
                <i class="fas fa-trophy"></i>
            </div>
            Lifesports
        </a>

        <ul class="nav-links">
            <li><a href="../public/index.html">Beranda</a></li>
            <li><a href="../public/lapangan.html">Lapangan</a></li>
            <li><a href="../public/index.html#cara-booking">Cara Booking</a></li>
            <li><a href="../public/index.html#kontak">Kontak</a></li>
        </ul>

        <div class="nav-actions">
            <a href="<?= BASEURL; ?>/auth/login" class="btn-nav">
                <i class="fas fa-sign-in-alt"></i> Masuk
            </a>
            <a href="<?= BASEURL; ?>/auth/register" class="btn-nav">
                <i class="fas fa-user"></i> Daftar
            </a>
        </div>
    </header>

    <main class="booking-page container">
        <a href="../public/lapangan.html" class="auth-back inline-back">
            <i class="fas fa-arrow-left"></i> Kembali ke daftar
        </a>

        <section class="booking-detail">
            <div class="booking-gallery">
                <img src="<?= BASEURL; ?>/public/assets/img/bola.png" alt="Gelora Bung Karno Arena" class="booking-cover">
                <div class="booking-thumbs">
                    <img src="<?= BASEURL; ?>/public/assets/img/bola.png" alt="Lapangan futsal">
                    <img src="<?= BASEURL; ?>/public/assets/img/basket indoor.jpg" alt="Lapangan indoor">
                    <img src="<?= BASEURL; ?>/public/assets/img/tenis2.jpeg" alt="Area olahraga">
                </div>
            </div>

            <div class="booking-info">
                <div class="venue-topline">
                    <span>Futsal</span>
                    <strong><i class="fas fa-star"></i> 4.9</strong>
                </div>
                <h1>Kedaton Futsal Arena</h1>
                <p class="booking-location"><i class="fas fa-map-marker-alt"></i> Kedaton, Bandar Lampung</p>
                <p class="booking-copy">Lapangan futsal indoor dengan permukaan sintetis, pencahayaan malam, ruang tunggu, parkir luas, dan akses mudah dari area utama Kedaton.</p>

                <div class="facility-list">
                    <span><i class="fas fa-shower"></i> Shower</span>
                    <span><i class="fas fa-car"></i> Parkir</span>
                    <span><i class="fas fa-wifi"></i> WiFi</span>
                    <span><i class="fas fa-mug-hot"></i> Cafe</span>
                </div>

                <div class="schedule-board">
                    <h2>Slot Tersedia</h2>
                    <div class="slot-grid">
                        <button>08:00</button>
                        <button class="active">09:00</button>
                        <button>10:00</button>
                        <button>13:00</button>
                        <button>15:00</button>
                        <button disabled>19:00</button>
                    </div>
                </div>
            </div>
        </section>

        <aside class="checkout-panel">
            <h2>Ringkasan Booking</h2>
            <form action="<?= BASEURL; ?>/pelanggan/pembayaran">
                <div class="form-group">
                    <label class="form-label" for="booking-date">Tanggal</label>
                    <input type="date" id="booking-date" class="form-control">
                </div>
                <div class="form-group">
                    <label class="form-label" for="duration">Durasi</label>
                    <select id="duration" class="form-control">
                        <option>1 Jam</option>
                        <option>2 Jam</option>
                        <option>3 Jam</option>
                    </select>
                </div>
                <div class="checkout-row">
                    <span>Harga per jam</span>
                    <strong>Rp 350.000</strong>
                </div>
                <div class="checkout-row">
                    <span>Biaya layanan</span>
                    <strong>Rp 5.000</strong>
                </div>
                <div class="checkout-total">
                    <span>Total</span>
                    <strong>Rp 355.000</strong>
                </div>
                <button type="submit" class="btn-primary btn-auth">
                    Bayar Sekarang
                </button>
            </form>
        </aside>
    </main>
</body>

</html>
